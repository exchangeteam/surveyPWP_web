...
class FormAns extends Component {
    ...
    handleSubmit = (e) => {
        e.preventDefault();
        this.props.form.validateFields(['title','desc'],{first:true},(err, values) => {
          if (!err) {
            console.log('Received values of form: ', values);
            axios.get('http://127.0.0.1:5000/api/questionnaires/').then(res =>{
                console.log(res)
            })
          }
        });
