...
import { Button, Icon, Form, Input, notification, Modal, message } from 'antd'
import './FormAns.css'
    ...
    handleSubmit = (e) => {
        ...
        that.props.form.validateFields(['title', 'description'], { first: true }, (err, values) => {
            if (!err) {
                ...
                axios.post(api.root + api.prefix + 'questionnaires/', { title: title, description: description }).then(res => {
                    console.log(res)
                    if (res.status === 201) {
